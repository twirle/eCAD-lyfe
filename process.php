...
if(isset($_GET["token"]) && isset($_GET["PayerID"])) {	
	...
	if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || 
	   "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
		...
		$qry = "SELECT * FROM shopcart WHERE shopcartID = $pid";
		$conn->query($qry);
		...
		$qry = "UPDATE shopcart SET Quantity=$_SESSION[NumCartItem], OrderPlaced=1,
				SubTotal=$_SESSION[SubTotal], ShipCharge=$_SESSION[ShipCharge],
				Tax=$_SESSION[Tax], Total=$total
		...
		if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || 
		   "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
			...
			$qry = "INSERT INTO orderdata (ShipName, ShipAddress, ShipCountry,
					...
					VALUES ('$ShipName', '$ShipADdress', '$ShipCountry',
